...
export function isKnownCombat(combat: unknown): combat is Combat.Known | null {
  // Check if the combat instance is valid
  return (
    combat === null || isClassicCombat(combat) || isTurnPassingCombat(combat)
  );
}

export function assertKnownCombat(
  combat: unknown,
): asserts combat is Combat.Known | null {
